...
exports.singleDayTime = async (req, res) => {
  try {
    ...
    // All check in checkouts of single day
    let checkinCheckoutArray = [];

    attendanceEntries.forEach((entry) => {
      totalMinInOffice += entry.duration;
      checkinCheckoutArray.push({
        checkinTime: moment(entry.checkinTime).format("hh:mm A"),
        checkoutTime: entry.checkoutTime
          ? moment(entry.checkoutTime).format("hh:mm A")
          : null,
      });
    });

    // console.log(attendanceEntries);
    console.log(checkinCheckoutArray);
